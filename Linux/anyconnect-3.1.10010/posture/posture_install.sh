...
LOGFNAME=`/bin/date "+posture-3.1.10010-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`/usr/bin/date "+posture-3.1.10010-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`date "+posture-3.1.10010-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=posture-3.1.10010-install.log
...
echo "Installing Cisco AnyConnect Posture Module v3.1.10010..." > /tmp/${LOGFNAME}
...
echo "Installing "${DIR}/libaccurl.so >> /tmp/${LOGFNAME}
${INSTALL} -o root -m 755 ${DIR}/libaccurl.so ${LIBDIR} || 
    ( echo "failed." && exit 1 )
